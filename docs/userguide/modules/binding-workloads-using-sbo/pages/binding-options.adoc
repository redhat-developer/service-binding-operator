...
== Binding workloads using a label selector
You can use a label selector to specify the workload that is being bound. If you declare a service binding using these label selectors to pick up workloads, the {servicebinding-title} periodically attempts to find and bind new workloads that match the given label selector.

For example, you may want to bind a service to every application with the `environment: production` label set in the `application` field. Doing so projects a single secret into multiple workloads.
...
* `name:` and `labelSelector` in the `binding.operators.coreos.com/v1alpha1` API
* `name:` and `selector` in the `servicebinding.io` API (Spec API)
