...
func TestAnnoationAndCSVBasedMetadata(t *testing.T) {
	...
	csv := mocks.ClusterServiceVersionMock(namespace, "cluster-service-version")
	err = f.Client.Create(context.TODO(), &csv, &framework.CleanupOptions{TestContext: ctx, Timeout: time.Second * 5, RetryInterval: time.Second * 1})
	require.NoError(t, err)
