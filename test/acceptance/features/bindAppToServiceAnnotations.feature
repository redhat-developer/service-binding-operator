            ...
            spec:
                services:
                  - group: stable.example.com
                    ...
                    name: cmsa-3-service
            """
        Then jq ".status.conditions[] | select(.type=="CollectionReady").status" of Service Binding "cmsa-3" should be changed to "True"
        And Secret "cmsa-3" does not have key "BACKEND"
        And Secret "cmsa-3" contains "BACKEND_CERTIFICATE" key with value "certificate value"
