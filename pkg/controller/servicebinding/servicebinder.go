...
var GroupVersion = v1alpha1.SchemeGroupVersion.WithResource(ServiceBindingResource)
...
// updateStatusServiceBinding updates the Service Binding Request's status field.
...
func buildBinding(
	...
	envVars, volumeKeys, err := NewRetriever(client).
		ProcessServiceContexts(globalEnvVarPrefix, svcCtxs, dataMapping)
	if err != nil {
		return nil, err
	}
