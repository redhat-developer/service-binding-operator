...
## -- Target for bumping up from a version 'n' to a version 'n+1'

.PHONY :version-update
## Upgrading operator version
version-update:
	@if [ ${OPERATOR_VERSION_UPDATE} != ${OPERATOR_VERSION} ]; \
	then \
		$(eval NEW_VERSION:=$(shell echo ${OPERATOR_VERSION_UPDATE} | tr '.' '\n' | tail -1)) \
		$(eval OLD_VERSION:=$(shell echo ${OPERATOR_VERSION} | tr '.' '\n' | tail -1)) \
		$(eval EXPECTED_VERSION:=$(shell echo ${OLD_VERSION} + 1 | bc )) \
		if [ ${NEW_VERSION} -eq  ${EXPECTED_VERSION} ]; \
		then \
			sed -i 's/${OPERATOR_VERSION}/0.0.${NEW_VERSION}/g' ./Makefile; \
			sed -i 's/${OLD_VERSION}/${NEW_VERSION}/g' ${MANIFESTS_DIR}/service-binding-operator.package.yaml; \
			sed -i 's/${OLD_VERSION}/${NEW_VERSION}/g' ${MANIFESTS_DIR}/service-binding-operator.v0.0.${OLD_VERSION}.clusterserviceversion.yaml; \
