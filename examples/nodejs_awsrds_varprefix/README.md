...
All we need to do is to create the following [`ServiceBinding`](./service-binding-request.shell-app.yaml):
...
* `application` - used to search for the application based on the resourceRef and the `resourceKind` of the application to be a `DeploymentConfig`
...
Create the following [`ServiceBinding`](./service-binding-request.nodejs-app.yaml):
...
The request is basically the same as the one we used for the Shell application. The only difference is the application name used in resourceRef used in the `application`.
