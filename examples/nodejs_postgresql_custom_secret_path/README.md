# Binding an Imported app to an In-cluster Operator Managed PostgreSQL Database with Custom Secret Path 

## Introduction

This scenario illustrates binding an imported application to an in-cluster operated managed PostgreSQL Database with Custom Secret Path.
...
spec:
  applicationSelector:
    bindingPath:
      customSecretPath: metadata.name
...
As you can see the custom secret path has been mentioned.
