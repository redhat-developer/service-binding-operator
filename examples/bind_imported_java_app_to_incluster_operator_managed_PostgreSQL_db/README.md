...
In this example we will import an arbitrary [Java Spring Boot application](https://github.com/ldimaggi/postgress-boot).
...
* `Git Repo URL` = `https://github.com/ldimaggi/postgress-boot`
* `Project` = `service-binding-demo`
* `Application`->`Create New Application` = `Java`
* `Name` = `postgress-boot-app`
* `Builder Image` = `Java`
...
``` bash
 kubectl patch dc postgress-boot -p '{"metadata": {"labels": {"connects-to": "postgres", "environment":"demo"}}}'
```

Alternatively, you can perform the same task with this make command:
``` bash
make set-labels-on-postgress-boot-app
```
